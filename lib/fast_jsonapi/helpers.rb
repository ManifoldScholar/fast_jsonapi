module FastJsonapi
  class << self
    ...
    def call_proc(proc, *params)
      ...
      # The format for parameters for most procs and lambdas is [:req | :opt, :param_name]
      # But the format for lambdas from symbols is [:param_name]
      # In the case of negative arity -- unlimited/unknown argument count --
      # just send the object to act as the method receiver.
      if proc.arity.negative? && proc.parameters.first.length == 1
